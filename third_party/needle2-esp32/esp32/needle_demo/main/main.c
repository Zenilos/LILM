/* Needle 2 on ESP32-S3: speech-shaped requests -> robot_action calls.
 *
 * The 14.65 MB mixed-width model lives in its own flash partition (0x1B0000,
 * see partitions.csv) and is memory-mapped, so the engine reads weights in
 * place and never copies them into RAM (there is nowhere near enough).
 * Activations and the KV cache go to octal PSRAM via ND_ALLOC.
 *
...
/* Key-presence test: optional slots are simply absent, and the grammar
 * constrains keys to declared properties, so strstr is exact. */
static int arg_key(const char *json, const char *key)
{
    char quoted[ND_GR_STRLEN + 4];

    snprintf(quoted, sizeof(quoted), "\"%s\"", key);
    return strstr(json, quoted) != NULL;
}

/* The onboard LED is the only actuator, so each intent maps to a distinct
 * pattern; the ACT line carries the full parsed action for the host. */
static void tool_robot_action(const char *args)
{
    char intent[ND_GR_STRLEN]  = "";
    char object[ND_GR_STRLEN]  = "";
    char recip[ND_GR_STRLEN]   = "";
    char loc[ND_GR_STRLEN]     = "";
    char file[ND_GR_STRLEN]    = "";
    char msg[ND_GR_STRLEN]     = "";
    char unit[ND_GR_STRLEN]    = "";
    float amount;

    if (!arg_str(args, "intent", intent, sizeof(intent))) {
        printf("ACT none\n");
        fflush(stdout);
        return;
    }

    /* Repair rule (see docs/V2_PLAN.md): the quantized model's one systematic
     * confusion is WAIT -> WAKEUP with the duration slots filled but no
     * recipient - an output the schema itself forbids (WAKEUP requires a
     * recipient). Rewrite it deterministically: measured 76% -> 88.5% on the
     * 200-query harness. */
    if (!strcmp(intent, "WAKEUP") && !arg_key(args, "recipient") &&
        arg_key(args, "duration_amount"))
        strcpy(intent, "WAIT");

    arg_str(args, "object", object, sizeof(object));
    arg_str(args, "recipient", recip, sizeof(recip));
    arg_str(args, "location", loc, sizeof(loc));
    arg_str(args, "file", file, sizeof(file));
    arg_str(args, "message", msg, sizeof(msg));
    arg_str(args, "duration_unit", unit, sizeof(unit));
    amount = arg_num(args, "duration_amount", 0.0f);

    printf("ACT robot_action intent=%s", intent);
    if (object[0])   printf(" object=%s", object);
    if (recip[0])    printf(" recipient=%s", recip);
    if (loc[0])      printf(" location=%s", loc);
    if (file[0])     printf(" file=%s", file);
    if (msg[0])      printf(" message=%s", msg);
    if (amount > 0)  printf(" duration=%.0f %s", amount, unit[0] ? unit : "?");
    printf("\n");
    fflush(stdout);

    if (!strcmp(intent, "CLEAN"))       led_run("blue",   "solid", 3);
    else if (!strcmp(intent, "GET"))    led_run("white",  "solid", 2);
    else if (!strcmp(intent, "GIVE"))   led_run("green",  "solid", 3);
    else if (!strcmp(intent, "MOVE"))   led_run("purple", "solid", 3);
    else if (!strcmp(intent, "PLAY"))   led_run("yellow", "flash", 3);
    else if (!strcmp(intent, "SHOW"))   led_run("blue",   "flash", 2);
    else if (!strcmp(intent, "UNAVAILABLE")) led_run("red", "solid", 4);
    else if (!strcmp(intent, "WAKEUP")) led_run("green",  "flash", 2);
    else if (!strcmp(intent, "WAIT")) {
        float secs = amount;
        if (unit[0] == 'm' && strncmp(unit, "ms", 2)) secs *= 60.0f;
        if (secs <= 0)        secs = 5;
        if (secs > 10)        secs = 10;   /* demo cap */
        led_run("yellow", "solid", secs);
    }
    /* STOP: no light - the point is that nothing is happening. */
}
...
static const struct {
    ...
    { "robot_action", tool_robot_action },
};
...
/* Route generated calls to their handlers. The grammar guarantees names are
 * ones the schema declared, so an unmatched name means the table and the
 * schema have drifted apart. Trained outputs pack several actions into one
 * array inside a single <tool_call> tag, so walk call objects one by one:
 * each starts at '{"name":"' and runs to just before the next such marker or
 * the closing tag. Handlers get a bounded copy because they scan with
 * strstr. */
#define CALL_SLICE_MAX 512

static void dispatch_call(const char *generated)
{
    const char *p = strstr(generated, "<tool_call>");
    const size_t nm = strlen("{\"name\":\"");
    ...
    for (;;) {
        const char *cs = strstr(p, "{\"name\":\"");
        const char *end_tag, *next, *args;
        char        name[ND_GR_STRLEN] = "";
        char        slice[CALL_SLICE_MAX];
        size_t      i, len;

        end_tag = strstr(p, "</tool_call>");
        if (!cs || (end_tag && cs >= end_tag))
            break;
        cs += nm;

        next = strstr(cs, "{\"name\":\"");
        if (end_tag && (!next || next > end_tag))
            next = end_tag;

        len = 0;
        while (cs[len] && cs[len] != '"' && len + 1 < sizeof(name)) {
            name[len] = cs[len];
            len++;
        }
        name[len] = '\0';
        ...
        for (i = 0; i < sizeof(TOOL_TABLE) / sizeof(TOOL_TABLE[0]); i++) {
            if (!strcmp(TOOL_TABLE[i].name, name)) {
                args = strstr(cs, "\"arguments\"");
                if (!args)
                    args = cs;
                len = (size_t)(next ? (size_t)(next - args) : strlen(args));
                if (len >= sizeof(slice))
                    len = sizeof(slice) - 1;
                memcpy(slice, args, len);
                slice[len] = '\0';
                TOOL_TABLE[i].fn(slice);
                break;
            }
        }
        if (i == sizeof(TOOL_TABLE) / sizeof(TOOL_TABLE[0])) {
...
void app_main(void)
{
    ...
    printf("EVT  e.g. \"clean the living room\"\n");
