...
#include <stdio.h>
#include <stdlib.h>
static int dbg_on(void){ static int v=-1; if(v<0){const char*e=getenv("ND_STAGE");v=(e&&e[0]=='1');} return v; }
static void dbg_vec(const char*tag,const float*v){ if(!dbg_on())return; printf("%s:",tag); for(int i=0;i<8;i++)printf(" %.4f",v[i]); printf("\n"); }
static void dbg_layer(const char*tag,const float*lane,int n,int dm){ if(!dbg_on())return; float acc[8]={0}; for(int i=0;i<dm;i++){float s=0;for(int j=0;j<n;j++)s+=lane[j*dm+i];if(i<8)acc[i]=s/n;} printf("%s:",tag); for(int i=0;i<8;i++)printf(" %.4f",acc[i]); printf("\n"); }
    ...
    for (s = 0; s < m->n_sites; s++) {
        ...
        /* k = key_proj @ e, raw v = value_proj @ e */
        {
            ...
            if (kp->bits == 2) {
                ...
                nd_cq_gemv(&m->c, kp, nd_cact_data(&m->c, kp), e,
                           m->row, m->eg_k + (size_t)s * dm);
                nd_cq_gemv(&m->c, vp, nd_cact_data(&m->c, vp), e,
                           m->row, vraw);
            }
        }
...
static void attention(nd_model *m, uint32_t li, const float *xin, float *out)
{
    ...
    if (L->q_proj.bits == 2) {
      ...
      nd_cq_prepare(&L->q_proj, xin, m->xh);
      nd_cq_gemv(&m->c, &L->q_proj, nd_cact_data(&m->c, &L->q_proj), xin, m->row, m->q);
      nd_cq_gemv(&m->c, &L->k_proj, nd_cact_data(&m->c, &L->k_proj), xin, m->row, m->kbuf);
      nd_cq_gemv(&m->c, &L->v_proj, nd_cact_data(&m->c, &L->v_proj), xin, m->row, m->vbuf);
      nd_cq_gemv(&m->c, &L->gate_proj, nd_cact_data(&m->c, &L->gate_proj), xin, m->row, m->gate);
    }
...
const float *nd_model_step_hidden(nd_model *m, uint32_t token)
{
    ...
    if (m->pos == 5 && dbg_on()) { dbg_vec("EMB", m->lane); dbg_vec("ek0", m->eg_k); dbg_vec("ev0", m->eg_v); }
    ...
    for (li = 0; li < m->n_layers; li++) {
        ...
        for (i = 0; i < dm; i++)
            m->u[i] -= m->ublk[i];
        if (m->pos == 5 && li == 0 && dbg_on()) { dbg_vec("L0y", m->u); }
    ...
    if (m->pos == 5 && dbg_on()) { dbg_layer("LASTLANE", m->lane, (int)m->lanes, (int)dm); }
    ...
    if (m->pos == 5 && dbg_on()) { dbg_vec("FINAL", m->y); }
